...
function addSearchCookie(q,link){var host=window.location.host;var hostList=host.split(".");hostList.splice(0,1,'//s');var mainUrl=hostList.join(".");var url=mainUrl+"/add_cookie?query="+encodeURI(q)+'&link='+encodeURI(link)
...
var base_url=hostList.join(".");var complex=$("#from_action").val();$('#keyword-input').change(function(){$('#st').val(1);});autocomplete_options={serviceUrl:PROTOCOL+'//www.tuniu.com/tn?r=search/search/searchSugguestV2',degreeUrl:PROTOCOL+'//www.tuniu.com/tn?r=search/search/searchSugguestRightV2',onSelect:autocomplete_onselect,isOuter:true};var autocomplete_a=$('#keyword-input').autocomplete(autocomplete_options);function autocomplete_onselect(i){var autocomplete=$(".autocomplete-w1").find(".resultbox");var autocomplete_href=autocomplete.eq(i).find("a").attr("href");window.open(autocomplete_href,"_self");}
...
request=$.ajax({url:"//www.tuniu.com/tn?r=homepage/site/CityQueryAjax",type:'GET',dataType:"jsonp",jsonp:"jsoncallback",data:{'query':i,'path':window.location?encodeURIComponent(location.protocol+'//'+location.host+location.pathname):"",'url':_this.patternUrl},success:function(json){if(json.data.cities==null){result.html('<p class="no_result">对不起，暂时没有找到结果</p>');result.show();}else{var jsonList=json.data.cities;result.html('');for(var i=0;i<jsonList.length;i++){result.append('<a href="'+jsonList[i].url+'">'+jsonList[i].cityName+'</a>');result.show();if(i==4){break;}}}},error:function(){}});}}};startCitySearch.init();},initScreenSize:function(){var userScreenSize={resizeScreen:function(){var index1200=$("body");if(!userScreenSize.isBigThan1240()){index1200.removeClass("index1200").addClass("index1000");userScreenSize.floatna1000();}else{index1200.removeClass("index1000").addClass("index1200");userScreenSize.floatna1200();}
...
function getKeyLink(queryKey,callback){$.ajax({url:"http://www.tuniu.com/tn?r=search/search/getKeyLink",type:"GET",dataType:"json",async:false,data:{query:queryKey},success:function(ret){if(ret&&ret.data){callback(ret.data);}else{callback(false);}},error:function(error){callback(false);}})}
function isRouteId(queryKey,callback){if(/^\d+$/.test(queryKey)){$.ajax({url:"http://www.tuniu.com/tn?r=search/search/isTour",type:"GET",dataType:"json",data:{id:queryKey},success:function(ret){if(ret&&ret.success){if(ret.data.isTour){if(ret.data.productType==1&&/^210\d{6}$/.test(queryKey)){callback(1)}else{callback(2)}}else{callback(false);}}else{callback(false);}},error:function(error){callback(false);}})}else{callback(false);}}
...
var base_url=hostList.join(".");var complex=$("#from_action").val();$('#keyword-input').change(function(){$('#st').val(1);});autocomplete_options={serviceUrl:PROTOCOL+'//www.tuniu.com/tn?r=search/search/searchSugguestV2',degreeUrl:PROTOCOL+'//www.tuniu.com/tn?r=search/search/searchSugguestRightV2',onSelect:autocomplete_onselect,isOuter:true};autocomplete_a=$('#keyword-input').autocomplete(autocomplete_options);function autocomplete_onselect(i){var autocomplete=$(".autocomplete-w1").find(".resultbox");var autocomplete_href=autocomplete.eq(i).find("a").attr("href");window.open(autocomplete_href,"_self");}
